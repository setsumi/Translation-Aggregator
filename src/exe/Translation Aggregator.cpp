...
void AutoTranslate(SharedString *string, int replaceUntrans) {
	if (string->length) {
		int hasJap = HasJap(string->string);
		SharedString *mod = ProcessSubstitutions(string);
		if (mod->length) {
			for (int i=0; i<numWindows; i++) {
				if (windows[i]->autoClipboard && windows[i]->hWnd) {
					if (!windows[i]->remote) {
						windows[i]->Translate(string, mod);
					}
			...
			if (replaceUntrans && srcWindow->hWndEdit) {
...
BOOL CALLBACK PlaceWindowFromMonitorEnum(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData) {
	...
	rects[1] = rects[0];
	if (rects[1].left < lprcMonitor->left || rects[1].left >= lprcMonitor->right) {
		rects[1].left = lprcMonitor->left;
	}
	if (rects[1].top < lprcMonitor->top || rects[1].top >= lprcMonitor->bottom) {
		rects[1].top = lprcMonitor->top;
	}
	if (rects[1].left + rects[1].right > lprcMonitor->right) {
	...
	if (rects[1].top + rects[1].bottom > lprcMonitor->bottom) {
...
/*BOOL WINAPI MyCreateProcessA(int x, int y, int z) {
	x=x;
	return 5;
}//*/
//#include <crtdbg.h>
...
/*#include <dll/hooks/HookManager.cpp>
#include <dll/hooks/disasm.cpp>
#include <shared/HookEval.h>

void GetCommandLineHookW(Registers *regs, void *param, wchar_t *id) {
	id = id;
	return;
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	/*
	HookRawAddress(GetCommandLineW, L"", 0, GetCommandLineHookW);
	GetCommandLineW();
	HookRawAddress(GetCommandLineW, L"", 0, GetCommandLineHookW);
	GetCommandLineW();
	HookRawAddress(GetCommandLineW, L"", 0, GetCommandLineHookW);
...
/*Registers regs;
	...
	regs.ebx = 5;
	int error = 0;
	int val = HookEval(&regs, L"(EDI-EBX*5/2+1)", &error);
	...
	ShellExecute(0, 0, L"AGTH.exe", 0, 0, 1);
	CreateProcessW(L"AGTH.exe", 0, 0, 0, 0, 0, 0, 0, &si, &pi);
	{
		STARTUPINFOA si;
		memset(&si, 0, sizeof(si));
		si.cb = sizeof(si);
		CreateProcessA("AGTH.exe", 0, 0, 0, 0, 0, 0, 0, &si, &pi);
	}
	HRESULT h = GetLastError();
	//MyCreateProcessA(1, 2, 3);
	//MyCreateProcessA(1, 2, 3);
	/*
	wchar_t *test = wcsdup(L"1\n3\n5");
	unsigned int rep[] = {2, 4};
	int goat = wcslen(test);
	LineBreakRemoveSome(test, goat, 1, 2);
	test=test;
	//*/
	//HookFunctions();
	//HookFunction(L"kernel32.dll", L"MultiByteToWideChar", 0, TextHook);
	//printf("test");
	//_CrtSetDbgFlag ( _CRTDBG_ALLOC_MEM_DF | _CRTDBG_LEAK_CHECK_DF );
	//_CrtSetDbgFlag(_CRTDBG_ALLOC_MEM_DF|_CRTDBG_CHECK_ALWAYS_DF|_CRTDBG_CHECK_CRT_DF|_CRTDBG_DELAY_FREE_MEM_DF|1);
	/*
	HookAfterFunction(GetCommandLineW, L"", 0, GetCommandLineHookW);
	HookAfterFunction(MessageBox, L"", 0, MyMessageBox);
	MessageBox(0, L"1", L"2", MB_OKCANCEL);
...
int WINAPI NoCRTMain() {
	return WinMain(GetModuleHandleW(0),0,0,0);
}
