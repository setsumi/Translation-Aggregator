...
wchar_t *AtlasTrans(wchar_t *jap, int len = -1);
extern wchar_t AtlasPath[2*MAX_PATH];
int LoadAtlasDlls();
wchar_t *TranslateFull(wchar_t *otext, int lineBreakFlags=~BREAK_ON_SINGLE_LINE_BREAKS, int freeText=0, int NeedAbort(int line, int lines, void *data)=0, void *data=0);
char *TranslateFull(char *otext, int lineBreakFlags=~BREAK_ON_SINGLE_LINE_BREAKS, int freeText=0, int NeedAbort(int line, int lines, void *data)=0, void *data=0);
