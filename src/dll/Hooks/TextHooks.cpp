...
void TextHook(Registers *regs, void *param, wchar_t *id) {
	...
	int error = 0;
	ULONG value = HookEval(regs, info->value, &error);
	int length = -1;
	if (info->length)
		length = HookEval(regs, info->length, &error);
	int codePage = 0;
	if (info->codePage) {
		codePage = HookEval(regs, info->codePage, &error);
	...
	int pos = 0;
	wchar_t **context = info->contexts;
	while (context[0]) {
		pos += wcslen(contextString+pos);
		wchar_t *append = L"::";
		if (context != info->contexts)
			append = L";";
		wsprintf(contextString+pos, L"%s%X", append, HookEval(regs, context[0], &error));
		context++;
	}
	if (error) {
		LeaveCriticalSection(&cSection);
